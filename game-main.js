// Lightweight wrapper: delegate to extracted core in `game_core.js`.
import { initCore, enable256Mode, disable256Mode, getModeState } from './game_core.js';

// Re-export mode helpers for compatibility with existing mode wrappers.
export { enable256Mode, disable256Mode, getModeState };

// Initialize core wiring when DOM is ready
if(document.readyState === 'loading'){
  document.addEventListener('DOMContentLoaded', ()=>{ try{ initCore(); }catch(e){ console.error('initCore failed', e); } });
} else {
  try{ initCore(); }catch(e){ console.error('initCore failed', e); }
}
async function doSwap(a,b){
  ...
  processing = true;
  // count this move for 256combo mode
  if(mode256combo){
    movesTaken = (typeof movesTaken === 'number' ? movesTaken : 0) + 1;
    updateStatus();
  }
  ...
  while(true){
    ...
    // update visible combo for this chain (use score state)
    try{ setCombo(localCombo); }catch(e){}
    updateStatus();
  ...
  if(localCombo>0){
    ...
    // if in 256combo mode, check whether we've reached target cumulative combos
    try{
      if(mode256combo && getComboBank() >= comboTarget256){
        const ov = el('endOverlay'); if(ov) ov.style.display = 'flex';
        try{ const esc = el('endScore'); if(esc) esc.textContent = `Score: ${String(getScore())}`; }catch(e){}
        try{ const rt = el('endRemovedTotal'); if(rt) rt.textContent = String(getState().removedTotal || 0); }catch(e){}
        try{ const cb = el('endComboBank'); if(cb) cb.textContent = String(getComboBank() || 0); }catch(e){}
        try{ const ml = el('endMovesLeft'); if(ml) ml.textContent = String(movesTaken); }catch(e){}
        try{ playSound('result'); }catch(e){}
        processing = false;
      }
    }catch(e){}
  }
  try{ setCombo(0); }catch(e){}
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  if(moveSel){
    const setLimit = (val)=>{
      ...
      // special 256combo mode
      if(String(val) === '256combo'){
        mode256combo = true;
        movesLimit = null; movesLeft = null;
        movesTaken = 0;
        uiLog('256comboモードが選択されました（目標: ' + String(comboTarget256) + ' コンボ）');
        updateStatus();
        return;
      } else {
        // if we were in 256combo mode and switched away, reset its state
        if(mode256combo){ mode256combo = false; movesTaken = 0; }
      }
...
function restartGame(){
  ...
  // reset 256combo counters (keep mode flag as-is)
  try{ movesTaken = 0; }catch(e){}
...
// Mode control helpers (exported so mode modules can toggle behavior)
export function enable256Mode(){
  try{ mode256combo = true; movesTaken = 0; uiLog('256comboモードを有効化しました'); updateStatus(); }catch(e){}
}

export function disable256Mode(){
  try{ mode256combo = false; movesTaken = 0; uiLog('256comboモードを無効化しました'); updateStatus(); }catch(e){}
}

export function getModeState(){
  return { mode256combo, movesTaken, comboTarget256 };
}
